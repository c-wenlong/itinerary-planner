...
class Form:
    ...
    def render_form(self):
        ...
        if submitted:
            ...
            skills = (
                [skill.strip() for skill in skills_input.split(",")]
                if skills_input
                else []
            )
            ...
            try:
                ...
                # REMOVE IN PROD
                self.visualise_output_json(activity=activity)
                # upload_activity(activity) ## uploads onto qdrant db

            except ValidationError as e:
                st.error(f"Validation error: {str(e)}")

    def visualise_output_json(self, activity: Activity):
        ...
        with col2:
            ...
            inferred_fields = self.openai.text_to_structured_output(
                activity.model_dump_json()
            )
            st.json(inferred_fields)

        st.subheader("Merged Activity")
        full_activity = self.merge_inferred_fields(activity, inferred_fields)
